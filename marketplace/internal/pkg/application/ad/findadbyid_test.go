...
func TestFindAdById_Execute_FindAd(t *testing.T) {
	...
	ad.SetId(anId)
	expectedResponse := GetAdByIdResponse{
		Id:          anId.String(),
	...
	ads.EXPECT().FindAdById(anId).Return(ad, nil)

	gotAd, _ := service.Execute(anId.String())
...
func TestFindAdById_Execute_NotFound(t *testing.T) {
	...
	anId, _ := NewId("574cc928-f4bd-11ed-ad0e-8a6a68a798d6")
	ads.EXPECT().FindAdById(anId).Return(Ad{}, errors.New("test: error"))

	gotAd, err := service.Execute(anId.String())
